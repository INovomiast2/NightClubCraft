...
/**
 * Pantalla para seleccionar un slot antes de crear un nuevo juego.
 */
public class SlotSelectionScreen extends Screen {
    ...
    public SlotSelectionScreen(Screen parent) {
        super(Text.literal("Select Business Slot"));
        this.parent = parent;
        
        // Inicializar slots
        for (int i = 0; i < TOTAL_SLOTS; i++) {
            slots.add(new SlotInfo(i, "Empty"));
        }
    }
    ...
    protected void init() {
        ...
        nextButton = this.addDrawableChild(ButtonWidget.builder(
            ...
            button -> {
                if (selectedSlot >= 0) {
                    // Abrir la pantalla de configuración de nuevo juego con el slot seleccionado
                    this.client.setScreen(new NewGameScreen(this, selectedSlot));
                }
            }
        ).dimensions(centerX - 210, this.height - 30, 200, 20).build());
        
        nextButton.active = false; // Deshabilitado hasta que se seleccione un slot
        
        this.addDrawableChild(ButtonWidget.builder(
            Text.literal("Cancel"),
            button -> this.close()
        ).dimensions(centerX + 10, this.height - 30, 200, 20).build());
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, Text.literal("Select a slot for your new business").styled(style -> style.withBold(true)), this.width / 2, 30, 0xCCCCCC);
